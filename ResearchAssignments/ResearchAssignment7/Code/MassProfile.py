...
class MassProfile:
    ...
    def Rad_MassEnclosed(self, ptype, m_percent):
        '''This function is meant to be the inverse of the previous, it takes a percent 
        of the total galaxy mass and determines a radius which encloses it.
        ...
        ----------
            ...
            m_percent: float [0-1]; the percent of total galaxy mass which 
            needs to be enclosed by the radius

        Returns
        -------
            r_crit: float [kpc]; the radius which encloses the desired 
            percentage of the galaxies mass as measured from the COM
        ...
        com = CenterOfMass(self.filename,2)
        # Calculates position of the COM with error tolerance 0.1 kpc
        com_pos = com.COM_P(self.volDec,0.1)
        # Indexes particles of a given type
        index = np.where(self.data['type'] == ptype)
        # Redefines the position of particles relative to the COM
        x_new = self.x[index] - com_pos[0]
        y_new = self.y[index] - com_pos[1]
        z_new = self.z[index] - com_pos[2]
        # Determines a distance from the COM for the particles      
        r_new = np.sqrt(x_new**2 + y_new**2 + z_new**2)
        #defines array of particle masses
        mG = self.m[index]
        
        sorted_indices = np.argsort(r_new) # sorts particles based ontheir distance from the COM
        sorted_r_new = r_new[sorted_indices] # reorders r_new to be by distance from COM
        sorted_mG = mG[sorted_indices] # sorts mass array by the same order
        
        cum_M = np.cumsum(sorted_mG) # uses cumulative sum to create array of mass up to given radius
        total_M = cum_M[-1] # total mass
        
        target_M = total_M * m_percent # determines what mass were looking for
        
        m_index = np.searchsorted(cum_M, target_M) # searches array for where this value can be inserted without changing the order
        
        r_crit = sorted_r_new[m_index] # uses found index to determine the radius which encloses percent mass
